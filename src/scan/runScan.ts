...
export interface RunScanInput {
  client: ModelClient;
  slots: CaptureSlots;
  // T13.3: aborting mid-scan rejects rather than resolving — see
  // src/model/abortable.ts for exactly what "aborting" can and can't stop.
  signal?: AbortSignal;
}
...
export async function runScan({ client, slots, signal }: RunScanInput): Promise<RunScanOutcome> {
  ...
  if (slots.ingredients) {
    ...
    const outcome = await detectIngredients(client, image, signal);
    if (!outcome.success) return { success: false, friendlyError: outcome.friendlyError };
    ingredients = outcome.result;
  }

  // Skip starting the nutrition call entirely if the scan was already
  // aborted while the ingredients call was in flight — no point kicking off
  // a second inference call the caller has already stopped waiting for.
  if (slots.nutrition && !signal?.aborted) {
    const image = await preprocessImage(slots.nutrition.uri);
    const outcome = await detectNutrition(client, image, signal);
    if (!outcome.success) return { success: false, friendlyError: outcome.friendlyError };
