...
function ensureModelLoaded(): Promise<void> {
  ...
  loadPromise = llm
    .loadModel(
      GEMMA_4_E2B_IT,
      {
        // backend: "cpu" avoids the memory-pressure teardown seen under
        // "gpu" on tight-RAM devices (POCO F1: torn down within ~20-60s by
        // Android's OS-level trim callback either way). It does NOT avoid
        // the separate "Failed to invoke the compiled model" crash on the
        // first real scan — HybridLiteRTLM.kt hardcodes the vision encoder
        // to GPU whenever multimodal:true is set regardless of this
        // setting, which is a package-level bug (open upstream, unfixed:
        // github.com/google-ai-edge/LiteRT-LM/issues/2461), confirmed live
        // on two separate physical devices, not a config problem this app
        // can work around. See plan.md T14.2 for the full trail.
        backend: "cpu",
        multimodal: true,
        // Default is 4096, sized for multi-turn chat. Every call this app
        // makes is one-shot (no conversation history — see generate()'s
        // resetConversation-before-every-call comment below), so the KV
        // cache doesn't need that much budget.
